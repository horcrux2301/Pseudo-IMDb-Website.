...
$(document).ready(function($) {
	console.log("hey");
	var url_for_trending_tv1 = 'https://api.themoviedb.org/3/tv/popular?api_key=' +  key +'&language=en-US&page=1';
	var url_for_trending_tv2 = 'https://api.themoviedb.org/3/tv/popular?api_key=' +  key +'&language=en-US&page=2';
	var url_for_trending_tv3 = 'https://api.themoviedb.org/3/tv/popular?api_key=' +  key +'&language=en-US&page=3';
	adddetails(url_for_trending_tv1);
	adddetails(url_for_trending_tv2);
	adddetails(url_for_trending_tv3);
	...
	function addthismovie(id,pop,title,poster,overview){
			if(i%3==0){
				...
				for(var tt=0;tt<overview.length && tt<150;tt++)
					desc+=overview[tt];
				desc+='....';
				var str = '<div class="row"><div class="col-md-4"> <div class="thumbnail"><img src="' + poster + '"alt="Poster_here"><div class="caption"><h3>' + title +'</h3><p>' + desc + '</p><p><a href="tv.html" class="btn btn-primary" role="button" onclick="setLocalStorage(' + id + ') ">Read More and Explore</a> </p></div></div></div></div>';
				console.log(str);
